...
import os,sys
from tkinter import OptionMenu,IntVar,_setit,StringVar
...
def create_window():
    ...
    root.sign_in_label = tk.Label(root, text='Loading...')
    root.sign_in_label.grid(row=0, column=0)
    root.sign_in_entry = tk.Entry(root)
    root.sign_in_entry.grid(row=0, column=1, sticky=tk.EW)
    root.sign_in_button = tk.Button(root, text='...',
                                            command= lambda: callbacks( sign_in))
    root.sign_in_button.grid(row=0, column=2)
    root.hide_button = tk.Button(root, text='Hide window',
                                            command= hide_window)
    root.hide_button.grid(row=0, column=3) 
...
def construct_advanced():
    ...
    root.speed_label.grid(row=2, column=0, sticky=tk.EW)
    root.speed_entry = tk.Entry(root,width=6)
    root.speed_entry.grid(row=2, column=2, sticky=tk.EW)
    root.command_entry = tk.Entry(root,width=6)
    root.command_entry.grid(row=2, column=1, sticky=tk.EW)
    root.iter_entry = tk.Entry(root,width=3)
    root.iter_entry.grid(row=2, column=3, sticky=tk.EW)
    root.settings_button = tk.Button(root, text='Set',
                                            command= update_settings)
    root.settings_button.grid(row=2, column=4)
    ...
    root.dropdown.config(width=20)
    root.dropdown.grid(row=1, column=0)

    root.button_del = tk.Button(root, text='Delete ',
                                            command=lambda: update_patterns(-3))
    root.button_del.grid(row=1, column=1)

    root.button_add_seq = tk.Button(root, text='Create new ',
                                            command=lambda: update_patterns(-4))
    root.button_add_seq.grid(row=1, column=4)
    ...
    root.pattern_label.grid(row=3, column=0, sticky=tk.EW)
    root.pattern_entry = tk.Text(root,width=10,height=10)
    root.pattern_entry.grid(row=4, column=0, sticky=tk.EW)

    root.button_minus = tk.Button(root, text='<--',
                                            command=lambda: show_data(-1))
    ...
    root.button_add = tk.Button(root, text='Add pattern',
                                            command=lambda: update_patterns(-1))
    root.button_add.grid(row=4, column=1)

    root.button_del = tk.Button(root, text='Delete last',
                                            command=lambda: update_patterns(-2))
    root.button_del.grid(row=4, column=2)

    root.button_save = tk.Button(root, text='Save pattern',
                                            command=lambda: update_patterns(0))
    root.button_save.grid(row=4, column=3)
...
def display_data(setting,pattern,n,patt_len,names):
    ...
    root.button_plus.configure(command=lambda:show_data(n+1))
    root.button_minus.configure(command=lambda:show_data(n-1))

    root.button_save.configure(command=lambda:update_patterns(n))
